...
generate_configs(){
    ...
    cd dhall/
    cp -rf ../../../examples/azure/client/gen/* .
    cd ../config
    mkdir nebula && cd nebula
    cp ../../../nest_system_ca.crt ../../../nebula .
    ...
    cd ../../
    rm nebula* *.crt *.key
    cp ../examples/azure/system/*.env .
    cp ../examples/azure/system/nest_service.yml nest_service/config/nebula/config.yml
    cp ../examples/azure/system/nest_ca.yml nest_ca/config/nebula/config.yml
    cp ../examples/azure/system/nest_config.yml nest_config/config/nebula/config.yml
    echo 1 > done_gen
}
